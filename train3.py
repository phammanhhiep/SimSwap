...
#############################################################
'''
Modified version of train.py, to test on training using different dataloader 
...
from data.data_loader_Swapping3 import GetLoader
    ...
    for step in range(start, total_step):
      ...
      for interval in range(2):
        ...
        '''My Note: even though called both lists of images as src (or source), 
          ...
          in a faceswap app.
        '''
        target_image, source_image, same  = train_loader.next()
        same = same.unsqueeze(-1).unsqueeze(-1).expand(target_image.shape)
        img_id = source_image
        ...
        if interval: # Train discriminator
          ...
          img_fake        = model.netG(target_image, latent_id)
          ...
          real_logits,_   = model.netD(source_image,None)
          ...
          real_feat       = model.netD.get_feature(target_image)
          ...
          loss_G_Rec  = model.criterionRec(
            torch.mul(img_fake, same), 
            torch.mul(target_image, same)) * opt.lambda_rec
          loss_G += loss_G_Rec
      ...
      # TODO: fix to adapt the new dataloader algorithm
      if (step + 1) % opt.sample_freq == 0:
        model.netG.eval()
        with torch.no_grad():
          imgs = list()
          zero_img = (torch.zeros_like(target_image[0,...])) # My Note: see the output for the reason of the black image
          imgs.append(zero_img.cpu().numpy())
          save_source_img = ((source_image.cpu())* imagenet_std + imagenet_mean).numpy()
          save_target_img = ((target_image.cpu())* imagenet_std + imagenet_mean).numpy()
          for r in range(opt.batchSize):
            imgs.append(save_source_img[r,...])
          arcface_112     = F.interpolate(source_image,size=(112,112), mode='bicubic')
          source_id  = model.netArc(arcface_112)
          source_id  = F.normalize(source_id, p=2, dim=1)

          for i in range(opt.batchSize):
            imgs.append(save_target_img[i,...]) 
            image_infer = target_image[i, ...].repeat(opt.batchSize, 1, 1, 1)
            img_fake    = model.netG(image_infer, source_id).cpu()
