...
def postprocess(swapped_face, target, target_mask, smooth_mask):
    ...
    mask_tensor = torch.from_numpy(
        target_mask.copy().transpose((2, 0, 1))
        ).float().mul_(1/255.0).cuda()
...
def reverse2wholeimage(
    ...
    Args:
        b_align_crop_tenor_list (TYPE): detected faces in the target images
        swaped_imgs (TYPE): swapped faces
        mats (TYPE): meta data used to detect faces
        crop_size (TYPE): Description
        oriimg (TYPE): target 
        logoclass (TYPE): Description
        save_path (str, optional): Description
        no_simswaplogo (bool, optional): Description
        pasring_model (None, optional): Description
        norm (None, optional): Description
    ...
    for swaped_img, mat ,detected_face_img in zip(swaped_imgs, mats,b_align_crop_tenor_list):
        ...
        if use_mask:
            norm_detected_face_img = norm(detected_face_img)
            norm_detected_face_img  = F.interpolate(norm_detected_face_img,size=(512,512))
            pasring_model_out = pasring_model(norm_detected_face_img)[0]
            pasring_max_index = pasring_model_out.squeeze(0).detach().cpu().numpy().argmax(0)
            vis_parsing_anno = pasring_max_index.copy().astype(np.uint8)
            ...
            if tgt_mask.sum() >= 5000:
                ...
                target_image_parsing = postprocess(
                    swaped_img, 
                    detected_face_img[0].cpu().detach().numpy().transpose((1, 2, 0)),
                    target_mask,
                    smooth_mask)
